...
export function TierCard({ tier, isCurrent, isDowngrade, loading, onSelect, recommended }: TierCardProps) {
  ...
  const config: Record<SubscriptionTier, {
    ...
    scrum_master: {
      ...
      price: `€${TIERS.scrum_master.price.replace('.', ',')}`,
    ...
    agile_coach: {
      ...
      price: `€${TIERS.agile_coach.price.replace('.', ',')}`,
      priceNote: t('tierPerMonth'),
      teams: t('tierTeams10'),
    ...
    transition_coach: {
      ...
      price: `€${TIERS.transition_coach.price.replace('.', ',')}`,
      priceNote: t('tierPerMonth'),
      teams: t('tierTeams25'),
      ...
      <div className="flex-1 mb-5">
        {isPaid ? (
          <div className="space-y-2">
            ...
            {TIERS[tier].features.crossTeam && (
              <div className="flex items-center gap-2">
                <svg className="w-4 h-4 text-green-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                </svg>
                <span className="text-sm text-stone-600 dark:text-stone-300">{t('tierFeatureCrossTeam')}</span>
              </div>
            )}
          </div>
        ) : (
